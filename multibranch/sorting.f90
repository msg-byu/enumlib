...
!****************************************************************************************************
! Uses heapsort to order a group of "records" (list) using a set of "keys" (the values to use in 
! sorting). See Knuth "Sorting and Searching" pgs. 142--147. In Knuth's discussion, rearrangments
! of the list also imply rearranging the keys. This is not explicit in Knuth's discussion.
...
!See above and see Knuth "Sorting and Searching" pgs. 142--147.
SUBROUTINE heapsort_records_dp(list,key) ! **** Same as above except keys are dp reals instead of integers
integer, pointer :: list(:)! indices of the records to be rearranged using the keys (R in Knuth)
real(dp), intent(inout):: key(:)! values on which to sort, the "keys" (K in Knuth)
...
do ! H2: decrease l or r
   ...
   do; i = j; j = 2*j
      if (j<r) then ! H5: Find larger child
         if (key(j) < key(j+1)) j = j + 1  ! This might not be a "stable sort" if we don't 
                                           ! account for finite precision here...
         if (pK > key(j)) exit ! Go to step H8
      else if(j==r) then! H6: Larger than pK?
         if (pK > key(j)) exit ! Go to step H8
      else
