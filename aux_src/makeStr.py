...
def _gaussian_reduce_two_vectors(U, V, eps):
    ...
    while not done:
        if it > 10:  # pragma: no cover
            ...
            exit()
        R = [
            V[i] - int(round(dot(U, V) / dot(U, U) + 1e-10)) * U[i] for i in range(3)
        ]  # Shorten V as much as possible
...
def _minkowski_conditions_check(basis, eps):
    ...
    if linalg.norm(b3) > (
        linalg.norm([b1[i] + b2[i] + b3[i] for i in range(len(b2))]) + eps
    ):
        minkowski_check = False
...
def _reduce_C_in_ABC(A, B, C, eps):
    ...
    if idx == 0:
        ...
        err(
            "Case failed in reduce_C_in_ABC"
            "Lattice coordinates in the A,B plane: ".format(
                " ".join([str(i) for i in LC])
            )
        )
...
def _get_lat_param_element(lat_vecs, n_basis_atoms, element):
    ...
    return atom_vol ** (1.0 / 3.0)


def _get_lattice_parameter(
    elements,
    concentrations,
    lat_vecs,
    n_basis_atoms,
    ...
    if elements is None:
        ...
        if len(elements) != len(concentrations):
            raise ValueError(
                "You have provided {0} element names when {1} elements are present "
            ...
            for i, elem in enumerate(elements):
                lat_param += concentrations[i] * _get_lat_param_element(
                    lat_vecs, n_basis_atoms, elem
                )
...
def _map_enumStr_to_real_space(system_data, structure_data, minkowskiReduce):
    ...
    if ic != n * nD:  # pragma: no cover
        err(
            "ERROR: map_enumStr_to_real_space: Didn't find the correct # of basis atoms"
        )
        exit()
...
def _minkowski_reduce_basis(IN, eps):
    ...
    if not _minkowski_conditions_check(OUT, eps):  # pragma: no cover
        err(
            "ERROR in minkowski_reduce_basis: Minkowski conditions not met."
    ...
    if args["structures"] == "all":
        ...
        if args["index_file"] == "":
            raise FileNotFoundError(
                "If the 'list' argument is used, the '-index_file' argument must be "
        ...
        if os.path.isfile(args["index_file"]):
            ...
            raise FileNotFoundError(
                "The specified index file, {}, containing structure numbers was not found "
    ...
    for line in structf:
        ...
        if not temp.startswith("#") and "#" not in temp.split()[0]:
            ...
            if line_count - (14 + adjust) in structures and adjust != 0:
                ...
                this_struct["HNF"] = [
                    [int(data[11]), 0, 0],
                    [int(data[12]), int(data[13]), 0],
                    [int(data[14]), int(data[15]), int(data[16])],
                ]
                this_struct["L"] = [
                    [int(data[17]), int(data[18]), int(data[19])],
...
def _write_config(system_data, space_data, structure_data, args, mapping=None):
    ...
    with open(filename, "a+") as poscar:
        ...
        for i in range(3):
            poscar.write(
                "   {}\n".format("      ".join(["{0: .11f}".format(j) for j in sLV[i]]))
            )
        poscar.write("  ")
        ...
        for iAt in range(structure_data["n"] * system_data["nD"]):
            for ilab in range(system_data["k"]):
                ...
                if labeling[gIndx[iAt]] == str(ilab):
                    ...
                    out_array = list(
                        array(dot(transpose(sLV), space_data["aBas"][iAt])) + displace
                    )
                    if mapping is None:
                        out_lab = ilab
                    else:
                    ...
                    poscar.write(
                        "             {0}    {1}       {2}\n".format(
                            iAt + 1,
                            out_lab,
                            "  ".join(["{0: .11f}".format(i) for i in out_array]),
                        )
                    )
        ...
        poscar.write(
            "Feature parent_lattice {}\n".format(system_data["title"])
        )  # This assumes the title in 'structenum.in' is the lattice type
        poscar.write(
            "Feature sym_size {}\n".format(structure_data["pgOps"])
        )  # Number of pointgroup operations for this crystal
        poscar.write(
            "Feature rescale {}\n".format(args["scale_factor"])
        )  # Scale factor for the volume
        poscar.write(
            "Feature set_id {}\n".format(args["set_id"])
        )  # This is a tag that can be set by the user when makeStr.py is called
        poscar.write(
            "Feature enum_id {}\n".format(structure_data["strN"])
        )  # This is the ordinal number in the structenum.out list
...
def _write_POSCAR(system_data, space_data, structure_data, args):
    ...
    with open(filename, "w+") as poscar:
        ...
        for i in range(3):
            poscar.write(
                " {}\n".format(" ".join(["{0: .9f}".format(j) for j in sLV[i]]))
            )
        poscar.write("  ")
            ...
            for iAt in range(structure_data["n"] * system_data["nD"]):
                ...
                if labeling[gIndx[iAt]] == str(ilab):
                    ...
                    poscar.write(
                        " {}\n".format(
                            "  ".join(
                                ["{0: .9f}".format(i) for i in out_array.tolist()]
                            )
                        )
                    )
...
def examples():
    ...
    explain = (
        "For all the examples below it is assumed you have already "
...
script_options = {
    "structures": dict(
        nargs="+",
        help=(
            "The desired structure numbers from the struct_enum.out file. This "
    ...
    "-displace": dict(
        ...
        type=float,
        help=(
            "The displacement amount for the arrows in units of the lattice "
            "parameter. Default is 0."
        ),
    ),
    "-input": dict(
        default="struct_enum.out",
        type=str,
    ...
    "-mink": dict(
        ...
        choices=["t", "f"],
        help=(
            "Sets flag to perform minkowski reduction of the basis (T/F)."
            " Default is True."
        ),
    ),
    "-species": dict(
    ...
    "-species_mapping": dict(
        ...
        type=int,
        help=(
            "Specify the atomic species numbers for the system. This option "
    ...
    "-outfile": dict(
        ...
        type=str,
        help=(
            "Override the default output file names: 'vasp.{structure#}'"
            "for the structures."
        ),
    ),
    "-rattle": dict(
        ...
        type=float,
        help=(
            "Randomizes the positions of the atoms in the POSCAR by no "
    ...
    "-config": dict(
        ...
        choices=["t", "f"],
        help=(
            "make an MTP config file instead of a VASP POSCAR. If the "
    ...
    "-scale_factor": dict(
        ...
        type=float,
        help=(
            "Rescales the lattice parameter. Multiplies the default (Vegard's law) by the "
    ...
    "-index_file": dict(
        default="",
        type=str,
        help=("Name of file containing a list of structure indices."),
    ),
    "-set_id": dict(
        default="unspecified",
        type=str,
        help=("Use 'set_id' to identify different sets of structures."),
    ),
}
...
def run(args):
    ...
    if (
        not RepresentsInt(args["structures"][0])
        and args["structures"][0].lower() == "all"
    ):
        args["structures"] = "all"
    elif (
        not RepresentsInt(args["structures"][0])
        and args["structures"][0].lower() == "list"
    ):
        ...
        args["structures"] = list(
            range(int(args["structures"][0]), int(args["structures"][1]) + 1)
        )
    else:
        raise ValueError(
