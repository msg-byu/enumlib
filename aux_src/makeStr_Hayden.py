...
def _get_lattice_parameter(
    elements, concentrations, lat_vecs, n_basis_atoms, default_title, remove_zeros=False
):
...
def _write_config(
    system_data, space_data, structure_data, args, mapping=None, conf_id=""
):
    ...
    with open(filename, "a+") as poscar:
        ...
        for i in range(3):
            poscar.write(
                "   {}\n".format("      ".join(["{0: .6f}".format(j) for j in sLV[i]]))
            )
...
def _write_POSCAR(system_data, space_data, structure_data, args):
    ...
    with open(filename, "w+") as poscar:
        ...
        for i in range(3):
            poscar.write(
                " {}\n".format(" ".join(["{0: .8f}".format(j) for j in sLV[i]]))
            )
...
def _make_structures(args):
    ...
    for structure in structure_data:
        ...
        if args["config"] == "f":
            ...
            _write_config(
                system, space_data, structure, args, args["mapping"], conf_id=_conf_id
            )
        _conf_id += 1
...
def examples():
    ...
    explain = (
        "For all the examples bellow it is assumed you have already "
