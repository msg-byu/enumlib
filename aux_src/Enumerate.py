...
from mlippy.makeStr import (
    _read_enum_out,
    _map_enumStr_to_real_space,
    _cartesian2direct,
    _get_lattice_parameter,
)
...
def parse_args():
    ...
    parser.add_argument(
        "yaml", nargs="+", help="One or more yaml files ready for enumeration"
    )
    parser.add_argument("--clear", type=bool, help="Clear previous database or not")
    ...
    def SetupProcess(self):
        ...
        for parent in self.lattice:
            ...
            with open(struct_enum, "w") as f:
                ...
                cell_size = [
                    ceil(dim / npoints) for dim in self.cell_size
                ]  # fixes size for multilattices
                print(cell_size)
...
def _write_ASE_OLD(system_data, space_data, structure_data, args):
    ...
    # user.
    lattice_parameter, _title = _get_lattice_parameter(
        args["species"],
        concs,
...
def write_ASE(system_data, space_data, structure_data, args):
    ...
    formula = "".join(
        [args["species"][i] + str(concs[i]) for i in range(len(args["species"]))]
    )
