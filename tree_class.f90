...
!!<summary>A "tree" object that contains all the members needed for enumerating derivative
!!structures at a fixed concentration. Translated from the python version "enum4" in the projects
!!git repository</summary>
type :: tree 
   private
   !!<member name="colors">  Number of each type (color); i.e., number of layers in the tree </member>
   !!<member name="k"> Number of different colors </member>
   !!<member name="n"> Total number of objects (number of "slots") in the enumeration </member>
   !!<member name="G"> Symmetry groups for each layer in the tree </member>
   !!<member name="Gsize"> Counter for the number of operations in each stabilizer (group at each layer)</member>
...
!!<summary>A list of group operations for each layer in the tree. I.e., the list of stabilizer groups</summary>

!!<summary>Stores a list of permutations (for the atomic sites in deriv. structure)</summary>
type :: permList
...
!!<parameter name="makeG" regular="true">Flag: True-> Generate group, False -> Generators already
  !!make a group</parameter>
!!<parameter name="uqperms" regular="true">Output. List of the symmetrically-distinct permutations.</parameter>
...
logical             :: makeG
integer, pointer    :: uqperms(:,:) ! Output from the routine, list of the permutations that are distinct

integer, allocatable:: labeling(:), rlabeling(:) ! Copy of labeling for current location and rotated
! (permuted) version of the same labeling according to one of the symmetry operations
...
allocate(tempPerms1(100,self%k)) ! Start with 100 possible survivors (doubles each time this isn't enough)
...
!   write(*,'(" || depth || ",i5)') self%depth()
   ...
   groupCheck: do ig = 1, self%Gsize(self%depth())
      ...
      if (all(rlabeling == labeling)) then ! this group op fixes the current labeling and is part of
         ...
         do i = 1, self%n
            if (labeling(i) < rlabeling(i)) then ! this rlabeling appeared earlier in the list
...
!!<parameter name="colors">A list of the numbers of each color in the enumeration</parameter>
!!<parameter name="generators">Symmetry operators (permutations) that generate the group (but may contain the whole
  !!group)</parameter>
!!<parameter name="makeG">Flag: True-> Generate group, False -> Generators already make a group</parameter>
...
enddo
! Initialize the group for layer 1. Generate the group if makeG = .true., otherwise assume that the
! list of generators given is the entire group
allocate(self%G%layer(self%k)) !Allocate number of perm lists. Don't need one for last color but
!makes the code cleaner. Perhaps rethink this later
...
!!<summary>Return the index for the current color (i.e., the current layer in the tree)</summary>
function indexOfCurrentColor(self)
class(tree), intent(in) :: self
integer    :: indexOfCurrentColor

indexOfCurrentColor = minloc(self%loc,1,self%loc==-1) -1 ! Current location has -1's for all layers
! below current depth
end function indexOfCurrentColor


!!<summary>Convert an integer to binary labeling. Takes the index of the current coloring, number of open
!!slots, and number of this color. There are n (m) choose k (a) possible configurations and the
!!integer (y) is the index in that list of configurations.</summary>
!!<comments>Follows the algorithm in the enum3 paper, Comp Mat Sci 59 101 (2010) exactly</comments>
...
!!or down between levels. When possible downward moves are first, then lateral moves, lastly
...
!!<summary>Advance the tree to the next branch. Move up to a higher level if the branches at this
!!level are exhausted</summary>
